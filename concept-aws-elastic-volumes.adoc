...
** Vous avez créé un nouveau système Cloud Volumes ONTAP depuis l'interface utilisateur. Dans ce cas, la fonctionnalité Elastic volumes est désactivée sur l'agrégat initial. Révision  ci-dessous pour en savoir plus.
...
+
Si vous créez un volume qui utilise des disques gp3 ou io1, BlueXP crée automatiquement le volume sur un agrégat sur lequel Elastic volumes est activé. Pour plus de détails, reportez-vous à  happens when you create a volume.
