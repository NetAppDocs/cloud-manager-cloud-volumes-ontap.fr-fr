...
* Un ensemble de règles pour les composants HA dans VPC-0. Ces règles permettent l'accès aux données à Cloud Volumes ONTAP.  rules for Cloud Volumes ONTAP,En savoir plus >>.
* Un autre ensemble de règles pour les composants HA dans les VPC-1, VPC-2 et VPC-3. Ces règles sont ouvertes pour les communications entrantes et sortantes entre les composants HA.  rules for Cloud Volumes ONTAP,En savoir plus >>.
...
Dans GCP, une configuration haute disponibilité est déployée sur quatre VPC. Les règles de pare-feu nécessaires à la configuration haute disponibilité dans VPC-0 sont les suivantes  rules for Cloud Volumes ONTAP,Répertoriées ci-dessus pour Cloud Volumes ONTAP.
