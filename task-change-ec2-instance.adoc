...
TIP: Cette option est également disponible en entrant dans l'environnement de travail, en ouvrant le menu d'action et en sélectionnant *Modifier l'instance*.

. Si vous utilisez une licence PAYGO basée sur un nœud, vous pouvez éventuellement choisir une licence différente.
. Choisissez un type d'instance, cochez la case pour confirmer que vous comprenez les implications de la modification, puis cliquez sur *OK*.
